...
namespace DeferredPipeline
{
    ...
    public class DeferredRendererModel : ModelProcessor
    {
        ...
        [Description("If set, this file will be used as the normal map on the " + 
        "model, overriding anything found in the opaque data.")]
        [DefaultValue("")]
        ...
        [Description("This will be the key that will be used to search the " +
            "normal map in the opaque data of the model")]
        ...
        new string[]
    {
            ...
            int vertexChannelIndex, ContentProcessorContext context)
        {
            ...
            // If this vertex channel has an acceptable names, process 
            //it as normal.
            if (acceptableVertexChannelNames.Contains(vertexChannelName))
            {
                base.ProcessVertexChannel(
                    geometry, 
                    vertexChannelIndex, 
                    context
                );
            }
            // otherwise, remove it from the vertex channels; it's just extra
            // data we don't need.
            else
            {
        ...
        private void LookUpTextures(NodeContent node)
        {
            ...
            if (mesh != null)
            {
                ...
                // If the NormalMapTexture property is set, we use that normal 
                // map for all meshes in the model.
                ...
                {
                    // If NormalMapTexture is not set, we look into the opaque 
                    // data of the model, and search for a texture with the 
                    // key equal to NormalMapKey
                    normalMapPath = mesh.OpaqueData.GetValue<string>(
                        NormalMapKey, 
                        null
                    );
                }
                // If the NormalMapTexture Property was not used, and the key 
                // was not found in the model, than normalMapPath would have 
                // the value null.
                if (normalMapPath == null)
                {
                    // If a key with the required name is not found, we make
                    // a final attempt, and search, in the same directory 
                    // as the model, for a texture named meshname_n.tga, 
                    // where meshname is the name of a mesh inside the model.
                    normalMapPath = Path.Combine(
                        directory, mesh.Name + "_normal.tga"
                    );
                ...
                {
                    // If SpecularMapTexture is not set, we look into the 
                    // opaque data of the model, and search for a texture 
                    // with the key equal to specularMapKey
                    specularMapPath = mesh.OpaqueData.GetValue<string>(
                        specularMapKey, 
                        null
                    );
                }
                ...
                {
                    // We search, in the same directory as the model, for a 
                    // texture named meshname_s.tga
                    specularMapPath = Path.Combine(
                        directory, 
                        mesh.Name + "_specular.tga"
                    );
                ...
                // Add the keys to the material, so they can be used by
                // the shader
                foreach (GeometryContent geometry in mesh.Geometry)
                {
                    // In some .fbx files, the key might be found in the 
                    // textures collection, but not in the mesh, as we checked 
                    // above. If this is the case, we need to get it out, and
                    ...
                    if (geometry.Material.Textures.ContainsKey(normalMapKey))
                    {
                        ExternalReference<TextureContent> texRef = 
                            geometry.Material.Textures[normalMapKey];
                    ...
                    {
                        ExternalReference<TextureContent> texRef =
                            geometry.Material.Textures[specularMapKey];
        ...
        [Description("If set, this file will be used as the specular map on " +
            "the model overriding anything found in the opaque data.")]
        [DefaultValue("")]
        ...
        [Description("This will be the key that will be used to search the " +
            "specular map in the opaque data of the model")]
        ...
        protected override MaterialContent ConvertMaterial(MaterialContent material,
            ContentProcessorContext context)
        {
            EffectMaterialContent deferredShadingMaterial = 
                new EffectMaterialContent();
            deferredShadingMaterial.Effect = 
                new ExternalReference<EffectContent>(
                    @"Shaders\DeferredRendering\RenderGBuffer.fx"
                );

            // Copy the textures in the original material to the new normal
            // mapping material, if they are relevant to our renderer. The
            // LookUpTextures function has added the normal map and specular 
            // map textures to the Textures collection, so that will be copied 
            // as well.
            foreach (
                KeyValuePair<String, ExternalReference<TextureContent>> texture
                in material.Textures)
            {
                if ((texture.Key == "Texture") ||
                        (texture.Key == "NormalMap") ||
                        (texture.Key == "SpecularMap"))
                    deferredShadingMaterial.Textures.Add(
                        texture.Key, 
                        texture.Value
                    );
            }
